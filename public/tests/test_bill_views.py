...
def test_bills_view_sponsor(client, django_assert_num_queries):
    ...
    with django_assert_num_queries(BILLS_QUERY_COUNT):
        assert len(client.get(f"/public/ak/bills?sponsor={amanda.id}").context["bills"]) == 1
...
def test_bills_view_classification(client, django_assert_num_queries):
    ...
    resolutions = len(client.get("/public/ak/bills?classification=resolution").context["bills"])
    assert len(client.get("/public/ak/bills?classification=constitutional+amendment").context["bills"]) == 2
...
def test_bills_view_subject(client, django_assert_num_queries):
    with django_assert_num_queries(BILLS_QUERY_COUNT):
        assert len(client.get(f"/public/ak/bills?subjects=nature").context["bills"]) == 2


@pytest.mark.django_db
def test_bills_view_status(client, django_assert_num_queries):
    with django_assert_num_queries(BILLS_QUERY_COUNT):
        assert len(client.get(f"/public/ak/bills?status=passed-lower-chamber").context["bills"]) == 1
